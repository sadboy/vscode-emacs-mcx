...
const makeSexpTravelFunc = (
    doc: TextDocument,
    pareditNavigatorFn: PareditNavigatorFn
) => {
    let src = doc.getText();
    ...
    public async execute(
        ...
        const newSelections = textEditor.selections.map((selection) => {
            ...
            return new Selection(
                isInMarkMode ? selection.anchor : newActivePosition,
                newActivePosition
            );
        });
...
export class MarkSexp extends EmacsCommand {
    ...
    public async execute(
        ...
        const navigatorFn =
            arg > 0
                ? paredit.navigator.forwardSexp
                : paredit.navigator.backwardSexp;
...
export class KillSexp extends KillYankCommand {
    ...
    public async execute(
        ...
        const travelSexp = makeSexpTravelFunc(
            doc,
            paredit.navigator.forwardSexp
        );
