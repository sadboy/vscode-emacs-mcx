 ...
 */
export class StartAcceptingRectCommand
    ...
    implements IEmacsCommandInterrupted
{
    ...
    public startAcceptingRectCommand(): void {
        ...
        vscode.commands.executeCommand(
            "setContext",
            "emacs-mcx.acceptingRectCommand",
            true
        );
    }
...
abstract class EditRectangle extends RectangleKillYankCommand {
    ...
    public async execute(
        ...
        const notReversedSelection = new vscode.Selection(
            selection.start,
            selection.end
        );
        ...
        if (this.copy) {
            const rectText = rectSelections.map((lineSelection) =>
                textEditor.document.getText(lineSelection)
            );
        ...
        await textEditor.edit((edit) => {
            ...
            while (rectLine <= rectHeight) {
                ...
                const additionalColumns = Math.max(
                    0,
                    insertColumn -
                        textEditor.document.lineAt(insertLine).range.end
                            .character
                );

                const insertText =
                    " ".repeat(additionalColumns) + killedRect[rectLine];
                edit.insert(
            ...
            if (rectLine <= rectHeight) {
                const additionalText = killedRect
                    ...
                    .map(
                        (lineText) =>
                            eolChar + " ".repeat(insertColumn) + lineText
                    )
                    .join("");
...
export class OpenRectangle extends EmacsCommand {
    ...
    public async execute(
        ...
        const starts = textEditor.selections.map(
            (selection) => selection.start
        );

        const rectSelections = selections
            .map(
                convertSelectionToRectSelections.bind(null, textEditor.document)
            )
            .reduce((a, b) => a.concat(b), []);
        await textEditor.edit((edit) => {
            rectSelections.forEach((rectSelection) => {
                const length =
                    rectSelection.end.character - rectSelection.start.character;
...
export class StringRectangle extends EmacsCommand {
    ...
    public async execute(
        ...
        const replaceString = await this.minibuffer.readFromMinibuffer({
...
export class ReplaceKillRingToRectangle extends EmacsCommand {
    ...
    public async execute(
        ...
        const currentRect = convertSelectionToRectSelections(
            textEditor.document,
            selection
        );
        ...
        await textEditor.edit((edit) => {
            for (let i = lineStart; i <= lineEnd; i++) {
                ...
                if (rgn.end.character < insertChar) {
                    ...
                    edit.insert(
                        new vscode.Position(i, rgn.end.character),
                        " ".repeat(fill) + text
                    );
                } else {
        ...
        const finalCursor = new vscode.Position(
            finalCursorLine,
            finalCursorChar
        );
        textEditor.selections = [
            new vscode.Selection(finalCursor, finalCursor),
        ];
    }
}
