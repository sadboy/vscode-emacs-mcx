...
ABCDEFGHIJ`
        ...
        suite(
            ...
            () => {
                ...
                const moves = moveCommandIds.map(
                    (commandName): [string, () => Thenable<unknown> | void] => [
                        commandName,
...
ABCDEFGHIJ`
                        ...
                        await emulator.runCommand("yankPop");
                        assert.ok(
                            activeTextEditor.document.getText().includes("BAR")
                        );
                        assert.ok(
                            !activeTextEditor.document.getText().includes("FOO")
                        );
                    });
...
suite(
    "Kill and yank with multi cursor, killing at 2 cursors in different lines",
    () => {
        let activeTextEditor: vscode.TextEditor;
        let emulator: EmacsEmulator;
...
suite(
    "Kill and yank with multi cursor, killing at 2 cursors in one line",
    () => {
        let activeTextEditor: vscode.TextEditor;
        let emulator: EmacsEmulator;
