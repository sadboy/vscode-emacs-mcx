...
export class PrefixArgumentHandler {
    ...
    private updateState(
        newState: Partial<PrefixArgumentHandlerState>
    ): Promise<unknown> {
        ...
        const acceptingStateChanged =
            oldState.isAcceptingPrefixArgument !==
            this.state.isAcceptingPrefixArgument;
        const prefixArgumentChanged =
            oldState.isInPrefixArgumentMode !==
    ...
    public subsequentArgumentDigit(arg: number): Promise<unknown> {
        if (!this.state.isInPrefixArgumentMode) {
            logger.debug(
                `[PrefixArgumentHandler.subsequentArgumentDigit]\t Not in prefix argument mode. exit.`
            );
        ...
        if (!this.state.isAcceptingPrefixArgument) {
            logger.debug(
                `[PrefixArgumentHandler.subsequentArgumentDigit]\t Prefix argument input is not accepted.`
            );
            return Promise.resolve();
        }
        ...
        if (isNaN(arg) || arg < 0) {
            logger.debug(
                `[PrefixArgumentHandler.subsequentArgumentDigit]\t Input digit is NaN or negative. Ignore it.`
            );
            return Promise.resolve();
        }
    ...
    public universalArgument(): Promise<unknown> {
        if (
            this.state.isInPrefixArgumentMode &&
            this.state.prefixArgumentStr.length > 0
        ) {
            logger.debug(
    ...
    public digitArgument(arg: number): Promise<unknown> {
        if (isNaN(arg) || arg < 0) {
            logger.debug(
                `[PrefixArgumentHandler.digitArgument]\t Input digit is NaN or negative. Ignore it.`
            );
            return Promise.resolve();
        }
    ...
    public negativeArgument(): Promise<unknown> {
        if (this.state.prefixArgumentStr !== "") {
            logger.warn(
                `[PrefixArgumentHandler.negativeArgument]\t Invalid invocation of negative-argument.`
            );
            return Promise.resolve();
        }
