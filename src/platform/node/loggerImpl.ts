...
class VsCodeMessage extends TransportStream {
    ...
    public override async log(
        ...
        const selectedAction = await showMessage(
            message,
            ...this.actionMessages
        );
...
class NodeLogger implements ILogger {
    ...
    public configChanged(configuration: IConfiguration) {
        ...
        this.logger.transports[0]!.level =
            configuration.debug.loggingLevelForConsole;
        this.logger.transports[0]!.silent = configuration.debug.silent;
        this.logger.transports[1]!.level =
            configuration.debug.loggingLevelForAlert;
        (this.logger.transports[1] as VsCodeMessage).configuration =
            configuration;
