...
export function generateKeybindings(src: KeyBindingSource): KeyBinding[] {
    ...
    if (src.key) {
        ...
        if (src.keys) {
            throw new Error(
                `Both .key and .keys are provided while just one of them should be.`
            );
        }
    } else if (src.keys) {
        keys = src.keys;
        ...
        throw new Error(
            `Neither .key nor .keys are provided: ${JSON.stringify(src)}`
        );
    }
    ...
    if (src.when) {
        ...
        if (src.whens) {
            throw new Error(
                `Both .when and .whens are provided while just one of them should be.`
            );
        }
    } else if (src.whens) {
        whens = src.whens;
 ...
 */
                ...
                if (keystrokes.length === 1) {
                    ...
                    keybindings.push({
                        ...
                        when: addWhenCond(
                            when,
                            "config.emacs-mcx.useMetaPrefixEscape"
                        ),
                        ...(src.args ? { args: src.args } : {}),
                    });
                    keybindings.push({
                        ...
                        when: addWhenCond(
                            when,
                            "config.emacs-mcx.useMetaPrefixCtrlLeftBracket"
                        ),
                        ...(src.args ? { args: src.args } : {}),
                    });
                } else {
...
export function isKeyBindingSource(
    ...
    // Check for .command
    if (
        typeof maybeSrc.command !== "undefined" &&
        typeof maybeSrc.command !== "string"
    ) {
        return false;
    }
    ...
    // Check for .when
    if (
        typeof maybeSrc.when !== "undefined" &&
        typeof maybeSrc.when !== "string"
    ) {
        return false;
    }
